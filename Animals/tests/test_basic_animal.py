...
templates = list(can_action_templates.items()) + \
            list(cant_action_templates.items())

unknown_action_answer = f"My name is {STD_ANIMAL_NAME} and " \
                        f"I don't know what is \"unknown_action\"."
init_params_list = [
    {},
    {'name': 'test_animal'},
    {'energy': 300},
...
def animal_for_test():
    ...
    """
    return Animal(name=STD_ANIMAL_NAME,
                  energy_consumption=std_energy_consumption)
...
def test_animal_speak(animal_for_test,  # pylint: disable=W0621
                      action, expected):
    """
...
def test_animal_unknown_action(animal_for_test):  # pylint: disable=W0621
    """
    Test for comparing the speech of an animal
    in unknown action with a template.

    :param animal_for_test: Class of testing animal.
    """
    say = io.StringIO()
    with redirect_stdout(say):
        animal_for_test._do_action(  # pylint: disable=W0212
            action='unknown_action', print_text=True)

    assert say.getvalue().strip() == unknown_action_answer


@pytest.mark.parametrize('action', ['run', 'swim', 'fly'])
def test_animal_action_energy(animal_for_test,  # pylint: disable=W0621
                              action):
    """
    Basic test for comparing the energy consumption
    of an animal with a template.
    ...
    assert start_energy - end_energy \
           == animal_for_test.energy_consumption[action]
    ...
    assert all(
        (animal.name == init_params.get('name', 'Animal'),
         ...
         animal.energy_consumption == init_params.get('energy_consumption',
                                                      {}),
         animal._templates == init_params.get(  # pylint: disable=W0212
             'text_templates',
             animal._templates))  # pylint: disable=W0212
    )
